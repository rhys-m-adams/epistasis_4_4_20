...
def plot_local(num_muts, AA_ind, name, num_J, seed_ID, boundary, energy_fun):
    ...
    kn = 101
    en = 102
    Kbins = np.linspace(-9.5, -5, kn)
    Ebins = np.linspace(0, 1, en)
    H = np.zeros((kn-1, en-1))
    total_count = 0.
    viable_count = 0.
    area = 0.
    viable_count1 = 0.
...
num_muts = 10
num_J = int(np.sum(num_muts1==2) - 1)
out_name = sys.argv[1]

f1A = get_f1(A1, num_muts1, KD1, A_wt, limit=[logKD2PWM(-9.5), logKD2PWM(-5)])[0]
f1E = get_f1(A1, num_muts1, E1, A_wt, limit=[-1, 0.5])[0]

A_heatmap1_2, E_heatmap1_2 = sub_heatmap2(pos1, num_muts1, KD1 - f1A, E1 - f1E, seed_id)
A_heatmap1_2[~np.isfinite(A_heatmap1_2)]=0
boundary = np.array([-6,-6.5,-7,-7.5,-8,-8.5,-9, -9.5])
bound_energy = logKD2PWM(boundary) - A_wt
plot_local(num_muts, list(range(10)), out_name, num_J, seed_id, boundary, lambda x:energy_ind_fun2(x, bound_energy, A_heatmap1, E_heatmap1, A_heatmap1_2, E_heatmap1_2))
