...
ax = axes[0]
med_rep, pos, A, AA, A2, KD_lims, exp_lims = get_data(transform=lambda x:10**x, prefix='data_wide_bounds/', KD_lims=[-11, 0])
...
ax = axes[1]
med_rep, pos, A, AA, A2, KD_lims, exp_lims = get_data(transform=lambda x:x, prefix='data_wide_bounds/', KD_lims=[-11, 0])
KD = np.array(med_rep['KD'])

wt_val = KD[num_muts==0]
f1, x = get_f1(A, num_muts, KD, wt_val, limit=KD_lims)
plot_epistasis(KD, f1, num_muts, KD_lims, ax, plot_ytick=True, max_freq=2, logscale=True, make_cbar=True)
ax.set_xlabel(r'$K_d$')
ax.set_ylabel(r'PWM [M]')
labeler.label_subplot(ax,'B')

plt.savefig('contour_scales_no_bounds.pdf')
plt.close()
